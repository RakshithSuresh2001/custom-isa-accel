...
openXC7/imem.hex is a fixed bring-up test program (NOP padding + MLOAD/MMUL/MSTORE sequence + an ALU sanity check), not general-purpose firmware.
...
xc7a100t.bin (the nextpnr-xilinx chip database) is not committed to this repo — it's a large generated artifact (~152MB). Build it locally before running make:
bash
  export XRAY_DIR=<path_to>/prjxray-db
  python3 <path_to>/nextpnr-xilinx/python/bbaexport.py --device xc7a100tcsg324-1 --bba xc7a100t.bba

### Place the resulting xc7a100t.bin in fpga/openxc7/ before building.

PRJXRAY_DB in the Makefile defaults to a snap-install path (/snap/openxc7/x1/opt/nextpnr-xilinx/external/prjxray-db/artix7). Override it if your install differs:
bash
  make PRJXRAY_DB=/your/path/to/prjxray-db/artix7 all

### Build and flash:

bash
cd fpga/openxc7
make all      # synth -> PnR -> bitstream (arty_top.bit)
make flash    # program via OpenOCD/JTAG (requires board connected)
make clean    # remove generated artifacts

.gitignore addition:

openXC7/xc7a100t.bin
openXC7/xc7a100t.bba
openXC7/*.json
openXC7/*.fasm
openXC7/*.frames
openXC7/*.bit

(keeps the chipdb and all build outputs out of version control, since they're regenerated by make)
---
